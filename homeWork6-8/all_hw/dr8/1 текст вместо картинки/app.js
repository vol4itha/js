...
function handleClick(clickedButtonEvent) {
    ...
    let productButtonText = card.button.innerText;
    // 4. получаем текст на кнопке, которая внутри .product
    
    
    if (productButtonText == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (productButtonText == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let textRandom = "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Numquam porro temporibus unde";
